...
public sealed class TransmissionDownloadClient(IHttpClientFactory httpClientFactory) : DownloadClientBase
{
    ...
    {
        var method = action switch { "pause" => "torrent-stop", "resume" => "torrent-start", "delete" => "torrent-remove", "recheck" => "torrent-verify", _ => null };
        ...
        if (action == "delete") arguments["delete-local-data"] = false;
