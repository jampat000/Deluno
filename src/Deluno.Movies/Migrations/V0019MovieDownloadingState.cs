...
/// </summary>
public sealed class V0019MovieDownloadingState : SqliteSqlMigration
{
    public override int Version => 19;

    public override string Name => "movie_downloading_state";

    protected override string Sql =>
        """
        -- No index. ix_movie_wanted_state_library_status already covers the
        -- work-list query, and adding a second one on (wanted_status,
        -- downloading_since_utc) made things worse rather than better: SQLite
        -- started preferring it for the catalogue page's correlated pick and a
        -- page at twenty thousand titles went from milliseconds to 13.4
        -- seconds. Caught by SubtitleScaleBenchmark, which exists for exactly
        -- this.
        ALTER TABLE movie_wanted_state ADD COLUMN downloading_since_utc TEXT NULL;
