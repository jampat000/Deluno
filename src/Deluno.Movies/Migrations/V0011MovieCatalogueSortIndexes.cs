...
/// existing <c>ix_movie_entries_created_title</c> is <c>(created_utc DESC,
...
/// 0.04ms after.
/// </summary>
public sealed class V0011MovieCatalogueSortIndexes : SqliteSqlMigration
{
    public override int Version => 11;

    public override string Name => "movie_catalogue_sort_indexes";

    protected override string Sql =>
        """
        CREATE INDEX IF NOT EXISTS ix_movie_entries_created_id
            ON movie_entries (created_utc, id);

        CREATE INDEX IF NOT EXISTS ix_movie_entries_title_id
            ON movie_entries (lower(title), id);

        CREATE INDEX IF NOT EXISTS ix_movie_entries_year_id
            ON movie_entries (COALESCE(release_year, -1), id);

        CREATE INDEX IF NOT EXISTS ix_movie_entries_rating_id
            ON movie_entries (COALESCE(rating, -1), id);
        """;
}
