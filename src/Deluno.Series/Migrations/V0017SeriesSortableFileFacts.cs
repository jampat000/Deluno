...
/// <summary>
/// Size and quality, on the show row, so a library can be ordered by them.
///
/// <para><b>Why this exists.</b> A file's size and quality live on
/// <c>series_wanted_state</c>, which the catalogue page reaches through a
...
/// every show in the library and sorting the lot: a full scan wearing a seek's
...
/// either moves.</para>
/// </summary>
public sealed class V0017SeriesSortableFileFacts : SqliteSqlMigration
{
    public override int Version => 17;

    public override string Name => "series_sortable_file_facts";
    ...
    protected override string Sql =>
        ...
        ALTER TABLE series_entries ADD COLUMN primary_file_size_bytes INTEGER NULL;
        ALTER TABLE series_entries ADD COLUMN primary_quality_rank INTEGER NULL;

        CREATE INDEX IF NOT EXISTS ix_series_entries_size_id
            ON series_entries (COALESCE(primary_file_size_bytes, -1), id);

        CREATE INDEX IF NOT EXISTS ix_series_entries_quality_rank_id
            ON series_entries (COALESCE(primary_quality_rank, -1), id);
        ...
        CREATE INDEX IF NOT EXISTS ix_series_entries_bitrate_id
            ON series_entries (COALESCE(CAST(primary_file_size_bytes AS REAL) / NULLIF(runtime_minutes, 0), -1), id);
        ...
        UPDATE series_entries SET
            primary_file_size_bytes = (
                SELECT pick.file_size_bytes FROM series_wanted_state pick
                WHERE pick.series_id = series_entries.id
        ...
        CREATE TRIGGER IF NOT EXISTS trg_series_primary_facts_ai
        AFTER INSERT ON series_wanted_state
        BEGIN
            UPDATE series_entries SET
                primary_file_size_bytes = (
                    SELECT pick.file_size_bytes FROM series_wanted_state pick
                    WHERE pick.series_id = NEW.series_id
                    ORDER BY pick.has_file DESC, pick.quality_cutoff_met ASC, pick.library_id ASC
                    LIMIT 1
                ),
                primary_quality_rank = (
        ...
        CREATE TRIGGER IF NOT EXISTS trg_series_primary_facts_au
        AFTER UPDATE ON series_wanted_state
        BEGIN
            UPDATE series_entries SET
                primary_file_size_bytes = (
                    SELECT pick.file_size_bytes FROM series_wanted_state pick
        ...
        CREATE TRIGGER IF NOT EXISTS trg_series_primary_facts_ad
        AFTER DELETE ON series_wanted_state
        BEGIN
            UPDATE series_entries SET
                primary_file_size_bytes = (
                    SELECT pick.file_size_bytes FROM series_wanted_state pick
                    WHERE pick.series_id = OLD.series_id
                    ORDER BY pick.has_file DESC, pick.quality_cutoff_met ASC, pick.library_id ASC
                    LIMIT 1
                ),
                primary_quality_rank = (
