  ...
  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 px-4 py-6 backdrop-blur-sm">
    <div className="w-full max-w-2xl space-y-[var(--page-gap)] rounded-2xl border border-hairline bg-card p-5 shadow-2xl">
      ...
      <div className="grid gap-[var(--grid-gap)] md:grid-cols-2">
        ...
        {operation === "quality" ? <Field label="Quality profile" help="Set one quality profile for all selected titles."><Select value={qualityProfileId} onChange={(event) => onQualityProfileChange(event.target.value)}><option value="">Choose profile</option>{qualityProfiles.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select></Field> : null}
        ...
        {operation === "tags" ? <Field label="Tags" help="Comma-separated tags to apply to all selected titles."><Input value={tags} onChange={(event) => onTagsChange(event.target.value)} placeholder="e.g. favorites, weekend, 4k" /></Field> : null}
      ...
      {confirming && operation !== "renamePreview" ? <div className="rounded-xl border border-amber-400/40 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">Confirming will run this operation across {selectedTitleLabel}.</div> : null}
